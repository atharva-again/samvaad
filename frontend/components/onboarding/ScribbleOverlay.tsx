...
function calculateTooltipPosition(
	...
	const isInCenterX =
		targetCenterX > viewport.width * 0.3 &&
		targetCenterX < viewport.width * 0.7;
	...
	tooltipX = Math.max(
		padding,
		Math.min(tooltipX, viewport.width - tooltipWidth - padding),
	);
	tooltipY = Math.max(
		padding,
		Math.min(tooltipY, viewport.height - tooltipHeight - padding),
	);
					...
					>
						...
						{shortcut && (
							<p className="text-white/40 text-[13px] mb-4">
								Shortcut:{" "}
								<span className="text-white/60 font-mono text-[12px] bg-white/10 px-1.5 py-0.5 rounded">
									{shortcut}
								</span>
							</p>
						)}
						...
						<div className="flex items-center justify-between mt-4">
							...
							>
								<kbd className="w-5 h-5 rounded bg-white/10 flex items-center justify-center text-[11px] font-mono">
									S
								</kbd>
								<span>Skip</span>
							</button>
