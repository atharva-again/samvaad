...
import { API_BASE_URL, type ChatMessage, endVoiceMode } from "@/lib/api";
...
export function InputBar({
	...
	const botStoppedSpeakingTimeoutRef = useRef<NodeJS.Timeout | null>(null);
	const isBotSpeakingRef = useRef<boolean>(false);
	const voiceConversationIdRef = useRef<string | null>(null); // Track voice session conversation ID
	const { processFiles } = useFileProcessor();

	// Handle initial mode selection
	const handleModeSelect = useCallback((selectedMode: "text" | "voice") => {
	...
	}, [mode, handleModeSelect, toggleSourcesPanel]);
	...
	useEffect(() => {
		if (defaultMessage && onMessageConsumed) {
			console.debug(
				"[InputBar] Loading default message for edit:",
				defaultMessage,
			);
	...
	const [voiceState, setVoiceState] = useState<
		"listening" | "processing" | "answering" | "error"
	>("listening");
	...
	// Power User State - latency and PTT only (strictMode, persona, enableTTS now in global store)
	const [_latencyMs, setLatencyMs] = useState<number | null>(null);
	const [isPTTActive, setIsPTTActive] = useState(false);

	// Voice settings state
	const [_selectedMicId, _setSelectedMicId] = useState<string>();
	const [_selectedSpeakerId, _setSelectedSpeakerId] = useState<string>();
	...
	useRTVIClientEvent(RTVIEvent.Connected, () => {
		...
		try {
			...
			const roomUrl =
				(transport as any)?.roomUrl ||
				(transport as any)?._roomUrl ||
				(transport as any)?.daily?.roomUrl ||
				(transport as any)?._daily?.roomUrl ||
				(transport as any)?.properties?.url ||
				(typeof (transport as any)?.getRoomUrl === "function"
					? (transport as any).getRoomUrl()
					: null);

			console.debug("[InputBar] Transport room URL:", roomUrl);
			console.debug(
				"[InputBar] Transport object keys:",
				transport ? Object.keys(transport) : "N/A",
			);
	...
	useRTVIClientEvent(RTVIEvent.BotReady, () => {
		console.debug(
			"[InputBar] RTVIEvent.BotReady - setting isConnecting to false",
		);
	...
	const handleDisconnect = async () => {
		console.debug(
			"[InputBar] handleDisconnect called, currentRoomUrl:",
			currentRoomUrl,
		);
		...
		if (botStoppedSpeakingTimeoutRef.current) {
			...
			botStoppedSpeakingTimeoutRef.current = null;
		}
		...
		// Full reload from backend after voice session ends (Option B: Backend as Source of Truth)
		// Clears UI messages and fetches authoritative data from SamvaadLLMContext
		const voiceConvId = voiceConversationIdRef.current;
		setVoiceSessionActive(false);
		if (voiceConvId) {
			const { reloadFromBackend } = useConversationStore.getState();
			reloadFromBackend(voiceConvId);
	...
	useRTVIClientEvent(RTVIEvent.Metrics, (metrics: unknown) => {
	...
	const handleStartSession = useCallback(async () => {
		...
		if (!client) {
			console.debug(
				"[InputBar] cannot join yet: Pipecat client not initialized",
			);
			return;
		}
		...
		try {
			...
			if (
				...
				try {
					...
					toast.error(
						"Microphone permission denied. Please enable the mic to use Voice Mode.",
					);
					setMode("text");
					setIsConnecting(false);
					return;
			...
			voiceConversationId = conversationId || uuidv7();
			voiceConversationIdRef.current = voiceConversationId;

			if (!conversationId) {
				const { setCurrentConversation } = useConversationStore.getState();
				setCurrentConversation(voiceConversationId);
				console.debug(
					"[InputBar] Voice session started, waiting for first transcript to create conversation:",
					voiceConversationId,
				);
			}
			...
			await client.startBotAndConnect({
				...
				requestData: {
					conversation_id: voiceConversationId, // Always pass ID for message persistence
					session_id: "default",
			...
			console.debug("[InputBar] DEBUG: Sending strict_mode =", strictMode);
			...
			setIsSessionActive(true);
			setVoiceSessionActive(true, voiceConversationId);
			...
			if (!conversationId && voiceConversationId) {
				const store = useConversationStore.getState();
				useConversationStore.setState({
					conversations: store.conversations.filter(
						(c) => c.id !== voiceConversationId,
					),
					currentConversationId: null,
				});
				conversationCache.delete(voiceConversationId).catch(() => {});
				console.debug(
					"[InputBar] Cleaned up failed voice conversation:",
					voiceConversationId,
				);
			}
		}
	}, [client, conversationId, enableTTS, persona, strictMode, setVoiceDuration, setIsConnecting, setIsSessionActive, setVoiceSessionActive, setVoiceState, setErrorMessage, setMode, updateRoomUrl, endVoiceMode]);
	...
	useEffect(() => {
		...
		window.addEventListener("keydown", handleKeyDown);
		return () => window.removeEventListener("keydown", handleKeyDown);
	}, [mode, isSessionActive, isConnecting, handleStartSession]);

	// Track previous mode to detect actual mode switches (not just re-renders)
	const prevModeRef = useRef<"text" | "voice" | null>(null);
	...
	useEffect(() => {
		...
		// If switching OUT of voice mode to text, disconnect
		// Only trigger on actual mode CHANGE (voice → text), not on re-renders
		const wasVoice = prevModeRef.current === "voice";
		const isNowText = mode === "text";

		if (wasVoice && isNowText && (client || isSessionActive)) {
			console.debug(
				"[InputBar] Mode switched from voice to text - disconnecting",
			);
			(async () => {
				try {
	...
	useRTVIClientEvent(RTVIEvent.UserStartedSpeaking, (evt?: unknown) => {
	...
	useRTVIClientEvent(RTVIEvent.UserStoppedSpeaking, () => {
		...
		const trySendMessage = (attempt: number) => {
			...
			if (fullUserMessage) {
				console.debug(
					`[InputBar] Sending user message (attempt ${attempt}):`,
					fullUserMessage,
				);
	...
	useRTVIClientEvent(RTVIEvent.BotStartedSpeaking, (evt?: unknown) => {
		console.debug("[InputBar] RTVIEvent.BotStartedSpeaking", evt);
		setVoiceState("answering");
		isBotSpeakingRef.current = true;

		// Cancel any pending "send message" timeout - bot is still speaking
		if (botStoppedSpeakingTimeoutRef.current) {
			clearTimeout(botStoppedSpeakingTimeoutRef.current);
			botStoppedSpeakingTimeoutRef.current = null;
		}
	...
	useRTVIClientEvent(RTVIEvent.BotStoppedSpeaking, (evt?: unknown) => {
		...
		// Debounce: Wait 500ms to ensure bot is truly done speaking (not just between sentences)
		// If BotStartedSpeaking fires again within this window, the timeout will be cancelled
		botStoppedSpeakingTimeoutRef.current = setTimeout(() => {
			// Double-check bot isn't speaking (race condition guard)
			if (isBotSpeakingRef.current) {
				console.debug("[InputBar] Bot started speaking again, skipping send");
				return;
			}
			...
			if (fullResponse) {
				console.debug(
					"[InputBar] Sending aggregated bot response:",
					fullResponse,
				);
				const voiceSources =
					currentCitations.length > 0 ? currentCitations : undefined;
				if (voiceSources) {
					console.debug(
						"[InputBar] Attaching",
						voiceSources.length,
						"citations to voice message",
					);
				}
				onVoiceMessage?.({
					role: "assistant",
					content: fullResponse,
					sources: voiceSources as unknown,
				});
	...
	// Capture bot's LLM output - aggregate sentences and send when bot stops speaking
	useRTVIClientEvent(RTVIEvent.BotOutput, (data: unknown) => {
		...
		// Capture sentences BEFORE TTS filtering (spoken: false) to preserve citations
		// The spoken:true version has citations stripped by CitationTextFilter
		// Each sentence fires twice: spoken:false (raw LLM) then spoken:true (after TTS)
		const text = data?.text || data?.data?.text;
		const spoken = data?.spoken ?? data?.data?.spoken;

		// Use spoken === false to get raw LLM output WITH citations
		if (spoken === false && text && text.trim()) {
			// Aggregate the sentence - ensure proper spacing between sentences
			const trimmedText = text.trim();
			if (
				currentBotResponseRef.current &&
				!currentBotResponseRef.current.endsWith(" ")
			) {
				currentBotResponseRef.current += " ";
			}
			currentBotResponseRef.current += trimmedText;
			console.debug(
				"[InputBar] Aggregated bot text (with citations):",
				currentBotResponseRef.current,
			);
		}
	});
	...
	useRTVIClientEvent(RTVIEvent.Disconnected, () => {
		const roomUrl = currentRoomUrlRef.current;
		console.debug(
			"[InputBar] RTVIEvent.Disconnected - resetting session state, roomUrl:",
			roomUrl,
		);
		...
		if (roomUrl) {
			...
			endVoiceMode(roomUrl).then((result) => {
				console.debug(
					"[InputBar] endVoiceMode result (from Disconnected event):",
					result,
				);
			});
		}
	...
	useRTVIClientEvent(RTVIEvent.Error, (error: unknown) => {
					...
					>
						<textarea
							ref={(el) => {
								...
								const _isMounting = !textareaRef.current && el;
							...
							placeholder={
								isMobile ? "Ask anything..." : "Find answers to your curiosity"
							}
							rows={1}
