...
export function WelcomeScreen() {
	...
	const shortcuts = [
		{
			...
			label: "Text Mode",
			icon: MessageSquare,
			action: () => handleShortcut("text"),
		},
		{
			keys: `${modifier}+V`,
			label: "Voice Mode",
			icon: Mic,
			action: () => handleShortcut("voice"),
		},
		{
			keys: `${modifier}+A`,
			label: "Attach Files",
	...
	const features = [
		...
		{
			icon: Mic,
			label: "Fluid voice interactions with real-time transcription",
		},
	];
			...
			<div className="relative z-10 flex flex-col items-center text-center max-w-2xl w-full px-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
				...
				<div className="mb-8 md:mb-8 space-y-3">
					...
					<p className="text-lg md:text-xl text-text-secondary font-light tracking-wide max-w-lg mx-auto leading-relaxed">
						...
						<span className="text-indigo-400/90 font-normal">
				...
				<div className="flex flex-col gap-3 mb-8 md:mb-10 w-full max-w-md mx-auto">
					{features.map((feature) => (
						<div
							key={feature.label}
							className="flex items-center gap-3 px-2 py-1 group select-none"
						>
				...
				<div className="mt-6 md:hidden">
					<p className="text-xs text-white/30 italic">
						PS: You can swipe left to quickly access the sources panel and add
						more documents.
					</p>
				</div>
			</div>
