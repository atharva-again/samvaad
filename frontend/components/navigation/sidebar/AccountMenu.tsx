...
export function AccountMenu({ onClose, onLogout }: AccountMenuProps) {
    const menuItems = [
        ...
        { icon: <HelpCircle className="w-4 h-4" />, label: "Help", hasSubmenu: true, onClick: () => { } },
        { icon: <LogOut className="w-4 h-4" />, label: "Sign Out", onClick: onLogout, isDanger: true },
    ];
        ...
        <div className="py-2 min-w-[160px]">
            {menuItems.map((item, i) => (
                ...
                >
                    ...
                    {item.hasSubmenu && <ChevronRight className="w-3 h-3 text-white/30" />}
                </button>
            ))}
        </div>
    );
