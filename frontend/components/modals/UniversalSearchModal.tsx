...
export function UniversalSearchModal({ onSettingsOpen }: UniversalSearchModalProps) {
	...
	const [activeCategory, setActiveCategory] =
		useState<SearchCategory>("conversations");
	...
	const handleConversationAction = useCallback(
		async (action: ConversationAction["id"], result: SearchResult) => {
			...
			switch (action) {
				...
				case "pin":
					...
					try {
						...
						toast.success(
							conv.isPinned ? "Unpinned conversation" : "Pinned conversation",
						);
						setShowSubmenu(false);
					} catch (_error) {
		...
		[router, setSearchOpen, togglePinConversation, isPinning],
	);

	// Handle source actions
	const handleSourceAction = useCallback(
		async (action: SourceAction["id"], result: SearchResult) => {
			...
			switch (action) {
				...
				case "rename": {
					...
					const baseName =
						lastDot !== -1 ? source.name.slice(0, lastDot) : source.name;
				...
				case "toggle-rag": {
					...
					toast.success(
						isNowAllowed ? "Source enabled for RAG" : "Source disabled for RAG",
					);
					setShowSubmenu(false);
					break;
				}
		...
		try {
			if (selectedResult.category === "conversations") {
				...
				if (selectedResult.title.includes(".")) {
					const ext = selectedResult.title.slice(
						selectedResult.title.lastIndexOf("."),
					);
	...
	useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (state.showRenameInput) {
				if (e.key === "Escape") {
					...
					if (
						...
						state.renameValue.trim() &&
						!state.isRenaming
					) {
						const isConversation =
							state.selectedResult.category === "conversations";
						setIsRenaming(true);
							...
							renameFile(state.selectedResult.id, state.renameValue.trim())
								.then(() => {
									if (state.selectedResult) {
										updateSource(state.selectedResult.id, {
											name: state.renameValue.trim(),
										});
									}
									toast.success("Source renamed");
									setIsRenaming(false);
			...
			if (state.showSubmenu && state.selectedResult) {
				const isConversation =
					state.selectedResult.category === "conversations";
				const actionsList = isConversation
				...
				switch (e.key) {
					case "ArrowDown":
						...
						setSubmenuIndex((prev) =>
							prev < actionsList.length - 1 ? prev + 1 : 0,
						);
						break;
					case "ArrowUp":
						e.preventDefault();
						setSubmenuIndex((prev) =>
							prev > 0 ? prev - 1 : actionsList.length - 1,
						);
						break;
					case "Enter":
						e.preventDefault();
						if (isConversation) {
							handleConversationAction(
								CONVERSATION_ACTIONS[state.submenuIndex].id,
								state.selectedResult,
							);
						} else {
							handleSourceAction(
								SOURCE_ACTIONS[state.submenuIndex].id,
								state.selectedResult,
							);
						}
						break;
	...
	const highlightMatch = (text: string, query: string) => {
		...
		for (let i = 0; i < text.length; i++) {
			if (
				...
				text[i].toLowerCase() === lowerQuery[queryIdx]
			) {
				result.push(
									...
									{showRenameInput ? (
										<div className="px-4 py-4">
											<div className="flex items-center bg-white/5 border border-white/10 rounded-lg px-3 focus-within:border-white/20">
												...
												/>
												{selectedResult.category === "sources" &&
													selectedResult.title.includes(".") && (
														<span className="text-sm text-white/40 select-none ml-1">
															{selectedResult.title.slice(
																selectedResult.title.lastIndexOf("."),
															)}
														</span>
													)}
											...
											<div className="flex gap-2">
														...
														if (selectedResult && !isDeleting) {
															...
															try {
																if (
																	...
																	const conv =
																		selectedResult.originalData as Conversation;
																	await Promise.all([
																	...
																	const source =
																		selectedResult.originalData as SourceItem;
																...
																toast.error(
																	`Failed to delete ${selectedResult.category === "conversations" ? "conversation" : "source"}`,
																);
																setIsDeleting(false);
															}
														}
												...
												>
													{isDeleting ? (
														...
														<>
															Delete
															<kbd className="px-1.5 py-0.5 text-[10px] bg-red-500/20 text-red-300/60 rounded font-mono">
																↵
															</kbd>
														</>
													)}
										...
										CONVERSATION_ACTIONS.map((action, index) => {
											...
											return (
												<button
													...
													key={action.id}
													onClick={() =>
														handleConversationAction(action.id, selectedResult)
													}
										...
										SOURCE_ACTIONS.map((action, index) => {
											...
											return (
												<button
													...
													key={action.id}
													onClick={() =>
														handleSourceAction(action.id, selectedResult)
													}
													onMouseEnter={() => setSubmenuIndex(index)}
													className={cn(
														"w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors",
