...
export function usePlatform() {
    const [platform, setPlatform] = useState<{ isMac: boolean; modifier: string; isMobile: boolean }>({
        isMac: false,
        modifier: "Alt",
        isMobile: false,
    });
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            const isMac = /Mac|iPod|iPhone|iPad/.test(platformStr) || /Mac/.test(userAgent);
