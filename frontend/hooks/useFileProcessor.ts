...
import type { CachedFile } from "@/lib/cache/db";
...
export function useFileProcessor() {
	...
	const refreshSources = React.useCallback(async () => {
		try {
			...
			if (cachedFiles.length > 0) {
				console.log(
					"[FileProcessor] Cache HIT - showing",
					cachedFiles.length,
					"files",
				);
				...
				listFiles()
					.then(async (files) => {
						console.log(
							"[FileProcessor] Background sync - got",
							files.length,
							"files from server",
						);
	...
	const uploadSingleFile = async (
		...
		try {
			...
			if (result.error) {
				console.error(
					"[useFileProcessor] Backend processing error:",
					result.error,
				);
	...
	const processFiles = async (files: File[]) => {
		...
		if (files.length > 0) {
			...
			if (oversizedFiles.length > 0) {
				...
				toast.error(
					`File${oversizedFiles.length > 1 ? "s" : ""} too large (max 25MB): ${names}`,
				);
			}
				...
				validFiles.map(async (file) => {
					...
					if (nameMatch) {
						...
						try {
							...
							const contentMatch = currentSources.find(
								(s) => s.contentHash === fileHash,
							);
							if (contentMatch) {
								console.log(
									`[useFileProcessor] Content duplicate found: ${file.name} matches ${contentMatch.name}`,
								);
					...
					if (duplicateMatch) {
						duplicates.push({
							file,
							match: { name: duplicateMatch.name, id: duplicateMatch.id },
						});
					} else {
