...
import { type CachedConversation, type CachedMessage, db } from "./db";
...
export const conversationCache = {
	...
	async get(
		id: string,
		userId: string,
	): Promise<CachedConversationWithMessages | null> {
	 ...
	 * Deduplicates by content to handle ID mismatches between frontend/backend
	 */
	async appendMessages(
		...
		await db.transaction("rw", [db.conversations, db.messages], async () => {
			...
			const uniqueMessages = newMessages.filter(
				(m) => !existingContents.has(`${m.role}:${m.content.trim()}`),
			);

			if (uniqueMessages.length > 0) {
				await db.messages.bulkPut(uniqueMessages);
			}
	...
	async saveMessage(
		...
		await db.transaction("rw", [db.conversations, db.messages], async () => {
			...
			const existing = await db.conversations.get({
	...
	async trimMessages(
		...
		if (oldestKey.length > 0) {
			console.log(
				`[Cache] Trimming ${oldestKey.length} old messages from ${conversationId}`,
			);
