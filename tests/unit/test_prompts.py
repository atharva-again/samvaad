...
def test_get_persona_prompt_new_personas():
    ...
    assert "human friend" in PERSONAS["friend"]
    assert "domain expert" in PERSONAS["expert"]

def test_get_mode_instruction_strict():
    inst = get_mode_instruction(strict_mode=True)
    assert "CRITICAL INSTRUCTION: You use 'Strict Mode'" in inst
    assert "do not know" in inst

def test_get_mode_instruction_hybrid():
    inst = get_mode_instruction(strict_mode=False)
    assert "prioritize the context" in inst
    assert "CRITICAL INSTRUCTION" not in inst
